...
NSString * samplemultidevicetabbedapplication$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation samplemultidevicetabbedapplicationObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_samplemultidevicetabbedapplication);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_samplemultidevicetabbedapplication gc];
}
...
{
	// create samplemultidevicetabbedapplication global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "samplemultidevicetabbedapplication" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_samplemultidevicetabbedapplication = [[samplemultidevicetabbedapplicationObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_samplemultidevicetabbedapplication];
	
	NSString *_samplemultidevicetabbedapplicationNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _samplemultidevicetabbedapplicationNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_samplemultidevicetabbedapplication valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:samplemultidevicetabbedapplication$ModuleRequireFormat,code];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			@throw [NSException exceptionWithName:@"org.samplemultidevicetabbedapplication.kroll" reason:[NSString stringWithFormat:@"Module \"%@\" failed to leave a valid exports object",path] userInfo:nil];
		}
	...
	@throw [NSException exceptionWithName:@"org.samplemultidevicetabbedapplication.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
